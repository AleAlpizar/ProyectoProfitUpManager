...
-------------------------------------------------------------------
-- 1. Tablas base / lookup / seguridad
-------------------------------------------------------------------

-- Tipos de documento para vencimientos
...
CREATE TABLE dbo.UnidadAlmacenamiento (
    ...
    Activo         BIT NOT NULL CONSTRAINT DF_UnidAlm_Activo DEFAULT(1)
);
GO
...
CREATE TABLE dbo.Usuario (
    ...
    EstadoUsuario   NVARCHAR(10) NOT NULL
        CONSTRAINT DF_Usuario_EstadoUsuario DEFAULT ('ACTIVE'),
    CreatedAt       DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
    CreatedBy       INT NULL,
    UpdatedAt       DATETIME2 NULL,
    UpdatedBy       INT NULL,
    CONSTRAINT CK_Usuario_EstadoUsuario
...
-- Password reset (versión final con TokenHash)
...
-------------------------------------------------------------------
-- 2. Clientes, fidelidad y actividades
-------------------------------------------------------------------
...
CREATE TABLE dbo.Cliente (
    ...
    DescuentoPorcentaje  DECIMAL(5,2) NOT NULL 
        CONSTRAINT DF_Cliente_DescuentoPorcentaje DEFAULT (0),
    DescuentoDescripcion NVARCHAR(200) NULL,
    CONSTRAINT CK_Cliente_DescuentoPorcentaje_Range
...
CREATE TABLE dbo.DocumentoVencimiento (
    ...
    UpdatedAt                  DATETIME2 NULL,
    UpdatedBy                  INT NULL,
    CONSTRAINT FK_DocumentoVencimiento_Tipo 
        FOREIGN KEY (TipoDocumentoVencimientoID) REFERENCES dbo.TipoDocumentoVencimiento(TipoDocumentoVencimientoID)
);
GO

-------------------------------------------------------------------
-- 4. Inventario, productos y bodegas
-------------------------------------------------------------------
...
CREATE TABLE dbo.Producto (
    ...
    CONSTRAINT FK_Producto_Unidad 
        FOREIGN KEY (UnidadAlmacenamientoID) REFERENCES dbo.UnidadAlmacenamiento(UnidadID)
);
GO
...
CREATE TABLE dbo.Inventario (
    ...
    CONSTRAINT CHK_Inventario_CantidadNonNegative CHECK (Cantidad >= 0)
);
GO
...
CREATE TABLE dbo.Venta (
    ...
    CONSTRAINT CHK_Venta_TotalNonNegative CHECK (Total >= 0)
);
GO
...
-------------------------------------------------------------------
-- 7. Reportes y bitácora de errores
-------------------------------------------------------------------
...
-------------------------------------------------------------------
-- 8. Módulos y permisos (Inventario)
-------------------------------------------------------------------
...
-- Asignar / actualizar rol a usuario
...
BEGIN
  ...
  SELECT SesionID, UsuarioID, Token, ExpireAt 
  FROM dbo.Sesion 
  WHERE Token = @Token;
END
GO
...
-- Alta de productos (versión con Descuento)
...
-- Detalle de producto
CREATE OR ALTER PROCEDURE dbo.usp_Producto_Detalle
    @ProductoID INT
AS
BEGIN
    SET NOCOUNT ON;
...
-- Admin bootstrap (Admin / admin@profit.local / hash ya calculado)
...
IF NOT EXISTS (SELECT 1 FROM dbo.Usuario WHERE Correo = 'admin@profit.local')
BEGIN
    ...
    DECLARE @UsuarioAdminID INT = SCOPE_IDENTITY();

    EXEC dbo.usp_UsuarioRol_AssignOrUpdate
      @UsuarioID = @UsuarioAdminID,
      @NombreRol = 'Administrador',
      @AssignedBy = @UsuarioAdminID;
END
GO

-- Módulo Inventario y permisos base
...
IF @RolAdmin IS NOT NULL AND @ModInv IS NOT NULL
BEGIN
...
IF @RolEmpleado IS NOT NULL AND @ModInv IS NOT NULL
BEGIN
...
-- Unidades de almacenamiento base
...
-- Tipos de documento de vencimiento base
WITH Seeds AS (
